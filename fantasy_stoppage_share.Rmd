---
title: "Stoppage Points Share - AFL Fantasy"
...
Stoppage share is a market-share analysis where players are ranked based on their proportion of the team's total stoppage points.

<a href="fantasy_stoppage_points.html">Stoppage Points</a>

<a href="fantasy_transition_points.html">Transition Points</a>

<a href="fantasy_transition_share.html">Transition Points Share</a>
...
table <- readRDS("C:/Users/jpopo/Documents/Rstats/Breakdowns.rds") %>% 
  ...
  mutate(team_total = sum(Stoppages,na.rm=T)) %>% 
  mutate(Share = 100*Stoppages/sum(Stoppages,na.rm=T)) %>% 
  ungroup() %>% 
  group_by(season,playerId) %>% 
  mutate(AV = 100*sum(Stoppages,na.rm=T)/sum(team_total,na.rm=T)) %>% 
  ungroup() %>% 
